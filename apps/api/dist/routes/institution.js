...
function spaceKind(value) {
    return value.toUpperCase();
}
function permissionsForPreset(rolePreset) {
    const base = {
        institution: false,
        courses: false,
        robots: false,
...
async function audit(app, input) {
...
export async function registerInstitutionRoutes(app) {
    ...
    app.post("/v1/institution/staff", async (request) => {
        ...
        const user = await app.prisma.user.create({
            data: {
                ...
                email: body.email,
                passwordHash,
                permissions: permissionsForPreset(body.rolePreset),
                biography: "",
                avatarUrl: null
            }
        });
